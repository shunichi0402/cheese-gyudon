...
async function addAttendAll(classID, date, time, status, remarks = ''){

    if (classID && date && time && status){
        const classData = await database.ref(`class/${classID}/student`).get();
        const parseClassData = classData.val();

        for (const [key, value] of Object.entries(parseClassData)){
            addAttend(key, date, time, status, remarks);
        }

        console.log(parseClassData);
    } else {
        console.log('入力されていない情報があります。')
    }
...
function relocadClass(){
    database.ref('class').get().then(data => {
        ...
        document.getElementById('attend-all-class').innerHTML = innerHTML;
...
document.getElementById('add-attend-all-button').addEventListener('click', () => {
    const studentClass = document.getElementById('attend-all-class');
    const date = document.getElementById('attend-all-date');
    const time = document.getElementById('attend-all-time');
    const status = document.getElementById('attend-all-status');
    const remarks = document.getElementById('attend-all-remarks');
    addAttendAll(studentClass.value, date.value, time.value, status.value, remarks.value);
    // console.log(studentClass.value, date.value, time.value, status.value, remarks.value);
});
